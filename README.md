...
ディレクトリ構造は以下の三つの層で構成されています：
- プレゼンテーション層：ユーザーインターフェースの管理を行い、入力と出力の操作を担当します。  
ユーザーからの注文データの入力と最終的な合計金額の出力がここで行われます。
- ワークフロー層：処理の流れやビジネスロジックの実行順序を制御する役割を担当します。  
注文データを受け取り、ドメイン層を通じて必要な計算を行い、最終的な合計金額をプレゼンテーション層に渡す役割を果たします。
- ドメイン層： 仕様をに対応したオブジェクトの表現と、ビジネスロジックの実装を担当します。  
...
2. 変更に強い設計及びディレクトリ構造  
クリーンアーキテクチャを意識して、プレゼンテーション層、ドメイン層、ワークフロー層を分けて実装しています。  
各層の責務を明確にすることで処理の流れを追いやすくし、層内の変更が他の層へ与える影響を抑えることもできます。  
また、ドメインに関しても他の層の変更の影響を受けないため、仕様の実装箇所が安全に守られることが期待できます。  
3. ユニットテストの効率化
処理を細かいロジックに分割し、また一部関数型プログラミングを採用することで、各関数を独立してテストできるようになります。  
このアプローチにより、個々の関数が予期した通りに動作するかを詳細に検証でき、全体としてのテストの網羅性が向上します。  
4. ソースコードの可読性向上
一部関数型プログラミングの考え方を採用し、for文やif文を極力使わず関数の組み合わせで処理を行っています。  
...
2. データの不変性の担保
関数型プログラミングではデータは一度作成されると変更され無いため、データの不変性が担保されます。  
実装に取り入れることで、データの状態が変化することによる副作用が発生しにくくなります。
3. 型安全性の考慮
型安全性を意識しているので、種別の追加等に対しての変更をコンパイルでエラー検知できるようにしている。  
4. 「未分類の商品」という概念の導入  
「注文商品」から直接「税率別の商品」を分類するのではなく、一体資産かそうでないかのを表現した「未分類の商品」ドメインを導入することで、    
